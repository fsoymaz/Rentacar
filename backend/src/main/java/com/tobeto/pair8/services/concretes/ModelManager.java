...
public class ModelManager implements ModelService {
    ...
    public void delete(Integer id) {
        Model modelToDelete = modelRepository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Model bulunamadı, ID:" + id));
    ...
    public List<GetAllListModelResponse> getAll() {
        ...
        List<GetAllListModelResponse> modelResponses = models.stream()
                .map(model -> this.modelMapperService
                        .forResponse().map(model, GetAllListModelResponse.class))
